...
| `server.js` — the emulator in this repo | Implements this spec; used to verify the parse rules round-trip |
...
* The leading `#` on the first command is **required by the hardware**, not conventional.
  ...
  The emulator in this repo tolerates its absence; do not rely on that.
...
The emulator in this repo implements the above. Known deliberate gaps:
